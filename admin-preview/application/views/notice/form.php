<?php
/* 공고 등록/수정 폼 — 디자인 744:5901 기반
 * Title: "공고 등록" 36px Bold
 * 폼: gap-40
 * label: 24px Bold #363636 w-160 py-12, required: red *
 * input: h-60 border #a7a7a7 rounded-10 px-24 22px Regular black
 * radio: 36x36 SVG + 22px Regular black gap-16
 * file: 파일선택 btn(bg #003592 h-52 rounded-10 px-20 18px Medium white) + dropzone(h-200 bg #f8f9fb border dashed)
 * 하단: 취소(border #003592 w-346 h-80 rounded-10) + 등록(bg #003592 w-346 h-80)
 */
$isEdit = isset($notice) && !empty($notice);
$formTitle = $isEdit ? '공고 수정' : '공고 등록';
$submitLabel = $isEdit ? '수정' : '등록';
$formAction = $isEdit ? ($baseUrl . '/notice/update/' . $notice['id']) : ($baseUrl . '/notice/store');

$title = $isEdit ? htmlspecialchars($notice['title']) : '';
$publishDate = $isEdit ? (isset($notice['publish_date']) ? $notice['publish_date'] : date('Y-m-d')) : date('Y-m-d');
$publishTime = $isEdit ? (isset($notice['publish_time']) ? $notice['publish_time'] : '00:00') : '00:00';
$currentCat = $isEdit ? (isset($notice['category']) ? $notice['category'] : 'settlement') : 'settlement';
?>
<!-- 헤더 -->
...
<!-- 폼 -->
<form method="post" action="<?= $formAction ?>" id="noticeForm" enctype="multipart/form-data">
    <?php if ($isEdit): ?><input type="hidden" name="id" value="<?= $notice['id'] ?>"><?php endif; ?>
    <input type="hidden" name="mode" value="<?= $isEdit ? 'edit' : 'create' ?>">

    <div style="display:flex; flex-direction:column; gap:40px;">
        <!-- 필수 표시: 우측, 18px Medium -->
        ...
        <!-- 공고명: label w-160 + input h-60 border #a7a7a7 rounded-10 px-24 -->
        <div style="display:flex; gap:24px; align-items:center; height:60px;">
            <div style="width:160px; flex-shrink:0; padding:12px 0; display:flex; gap:4px; align-items:center;">
                <span style="font-size:24px; font-weight:700; color:#363636; line-height:1.5;">공고명</span>
                <span style="font-size:20px; font-weight:400; color:rgba(255,0,0,0.8); line-height:32px;">*</span>
            </div>
            <div style="flex:1; display:flex; flex-direction:column; gap:8px;">
                <input type="text" name="title" id="inputTitle" value="<?= $title ?>" placeholder="공고명을 입력해주세요." style="width:100%; height:60px; border:1px solid #a7a7a7; border-radius:10px; padding:0 24px; font-size:22px; font-weight:400; color:#000; line-height:1.6; font-family:var(--font); box-sizing:border-box;">
                <div id="titleError" style="display:none; font-size:16px; color:#e53e3e;"></div>
            </div>
        </div>

        <!-- 게시 일시: label + date(flex-1 h-60) + time(flex-1 h-60) -->
        <div style="display:flex; gap:24px; align-items:center;">
            ...
            <div style="flex:1; display:flex; gap:16px; align-items:center;">
                <div style="flex:1; position:relative;">
                    <input type="text" name="publish_date" id="noticeDate" value="<?= $publishDate ?>" class="picker-input" placeholder="YYYY-MM-DD" readonly>
                ...
                <div style="flex:1; position:relative;">
                    <input type="text" name="publish_time" id="noticeTime" value="<?= $publishTime ?>" class="picker-input" readonly>
        ...
        <!-- 구분: label + radio 3개 (결산공고/주주총회/기타) -->
        <div style="display:flex; gap:24px; align-items:center; height:60px;">
            <div style="width:160px; flex-shrink:0; padding:12px 0; display:flex; gap:4px; align-items:center;">
            ...
            <div style="display:flex; gap:24px; align-items:center;">
                <?php
                $categories = ['settlement' => '결산공고', 'shareholder' => '주주총회', 'etc' => '기타'];
                foreach ($categories as $val => $label):
                ?>
                <label style="display:flex; gap:16px; align-items:center; width:172px; cursor:pointer;">
                    <input type="radio" name="category" value="<?= $val ?>" <?= $currentCat === $val ? 'checked' : '' ?> style="display:none;" class="notice-radio">
        ...
        <!-- 첨부파일: label + 파일선택(bg #003592 h-52 rounded-10 px-20) + dropzone(h-200) + 안내 -->
        <div style="display:flex; gap:24px; align-items:flex-start;">
            ...
            <div style="flex:1; display:flex; flex-direction:column; gap:24px;">
                <!-- 파일선택 버튼: bg #003592 h-52 rounded-10 px-20 18px Medium white -->
                <div style="padding-top:4px;">
                    <label for="noticeFile" style="display:inline-flex; align-items:center; justify-content:center; height:52px; padding:0 20px; background:#003592; border-radius:10px; font-size:18px; font-weight:500; color:#fff; cursor:pointer; line-height:1.6;">
                        파일선택
                    </label>
                    <input type="file" name="file" id="noticeFile" style="display:none;" accept=".jpg,.gif,.png,.xls,.xlsx,.doc,.docx,.ppt,.pdf,.txt,.zip">
                    <span id="fileNameDisplay" style="margin-left:12px; font-size:18px; color:#363636;"></span>
                </div>
                <!-- 드롭존: bg #f8f9fb border dashed #a7a7a7 h-200 rounded-10 gap-10 -->
                <div id="noticeDropzone" style="width:100%; height:200px; border:1px dashed #a7a7a7; border-radius:10px; background:#f8f9fb; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:10px; padding:13px 24px;">
                ...
                <!-- 안내문: dot(6x6) + 20px Regular #7a7a7a -->
    ...
    <div class="form-actions" style="display:flex; gap:16px; align-items:center; justify-content:center; padding:60px 0;">
        <a href="<?= $baseUrl ?>/notice/list" id="cancelBtn" style="display:flex; align-items:center; justify-content:center; width:346px; height:80px; border:1px solid #003592; border-radius:10px; font-size:24px; font-weight:700; color:#003592; line-height:1.5;">
            취소
        </a>
...
(function(){
    ...
    // 날짜 피커 (flatpickr)
    var dateInput = document.getElementById('noticeDate');
    flatpickr(dateInput, {
        locale: 'ko',
        dateFormat: 'Y-m-d',
    ...
    // 시간 피커 (커스텀)
    new TimePicker('#noticeTime', {
        onChange: function() { dirty = true; }
    });

    // 라디오 이미지 토글
    document.querySelectorAll('.notice-radio').forEach(function(radio) {
    ...
    function showFileName(files) {
        ...
        // 드롭존을 Input Container(피그마 744:7427) 상태로 변환
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (!titleInput.value.trim()) {
            titleError.textContent = '공고명을 입력해 주세요.';
